...
export interface ImportRowData {
  ...
  position: string;
  strong_foot: string;
  number: string; // chaîne numérique ou ''
}
...
function normalizeNumber(value: unknown): { ok: boolean; value: string } {
  const s = cellToString(value);
  if (!s) return { ok: true, value: '' };
  const n = Number(s);
  if (!Number.isInteger(n) || n < 0 || n > 99) return { ok: false, value: s };
  return { ok: true, value: String(n) };
}

/**
 * Parse un fichier Excel d'import d'effectif. Ne fait aucune requête réseau —
 * pure fonction sur les octets du fichier. Les doublons contre l'effectif existant
 * sont marqués séparément par markExistingDuplicates (nécessite les joueurs déjà chargés).
 */
export function parsePlayersWorkbook(data: ArrayBuffer): ParsedPlayerRow[] {
