...
export const matchRatingsService = {
  ...
  async getCoachNotes(matchId: string): Promise<Record<string, number>> {
  ...
  async setCoachNote(matchId: string, playerId: string, note: number | null): Promise<void> {
    if (note === null) {
      const { error } = await supabase
        .from('match_player_coach_notes')
        .delete()
   ...
   * courbe page joueur / analytics. À joindre côté client avec les dates des matchs déjà
   * chargés (même pattern que getRatingsForMatches).
   */
  async getCoachNotesForMatches(matchIds: string[]): Promise<MatchPlayerCoachNote[]> {
    if (matchIds.length === 0) return [];
    const { data, error } = await supabase
      .from('match_player_coach_notes')
