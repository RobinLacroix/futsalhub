...
/**
 * Disponibilité — couche service web.
 *
 * Jumeau mobile : `mobile/lib/services/availability.ts`. Toute règle métier
 * ajoutée ici doit l'être là aussi : c'est la divergence silencieuse des paires
 * de services qui a produit les cinq bugs de données des match recorders.
 *
...
export const availabilityService = {
  ...
  async getClubAvailability(clubId: string, teamId?: string | null): Promise<AvailabilityRow[]> {
   ...
   */
  async setPlayerAvailability(input: SetAvailabilityInput): Promise<string> {
  ...
  async getPlayerAvailabilityHistory(
    playerId: string,
    from?: string | null,
  ...
  /**
   * Signaux précoces : N signalements de douleur sur la même zone et le même
   * côté pour un joueur, sur une fenêtre glissante.
   *
