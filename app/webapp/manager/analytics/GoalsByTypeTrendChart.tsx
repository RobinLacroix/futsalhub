/**
 * Évolution des buts par type, match par match. Miroir de
 * mobile/components/analytics/GoalsByTypeTrendChart.tsx.
 */
'use client';
  ...
  return matches
    ...
    .map((m) => {
      ...
      (eventsByMatch[m.id] ?? []).forEach((ev) => {
        const gt = (ev as unknown as { goal_type?: GoalType | null }).goal_type;
        if (!gt) return;
      ...
      const dateLabel = new Date(String(m.date)).toLocaleDateString('fr-FR', { day: 'numeric', month: 'numeric' });
      return {
        matchId: m.id,
        label: dateLabel,
        scored: s,
...
export function GoalsByTypeTrendChart({
  ...
  if (data.length === 0) {
    return <EmptyState icon={TrendingUp} title="Aucune donnée" description="Suivez des matchs au tracker avec le type de but renseigné." compact />;
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: theme.space.md, paddingLeft: theme.space.lg, paddingRight: theme.space.lg, marginBottom: 4 }}>
        {TYPES.map((t) => (
          <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: TYPE_COLORS[t] }} />
            <Text variant="caption" tone="tertiary">{TYPE_LABELS[t]}</Text>
          </div>
        ))}
      </div>
      <Text variant="caption" tone="tertiary" style={{ paddingLeft: theme.space.lg, paddingRight: theme.space.lg, marginBottom: theme.space.sm } as React.CSSProperties}>
        Couleurs vives = marqués · Estompés = encaissés
      </Text>
      <div style={{ overflowX: 'auto' }}>
        <svg width={totalW} height={svgH}>
          {gridVals.map((val) => (
            <line key={`gl${val}`} x1={PAD_LEFT} y1={baseY - toH(val)} x2={totalW - PAD_RIGHT} y2={baseY - toH(val)} stroke={c.border.subtle} strokeWidth={1} />
          ))}
          {gridVals.map((val) => (
            <text key={`gy${val}`} x={PAD_LEFT - 4} y={baseY - toH(val) + 4} fontSize={11} fill={c.text.tertiary} textAnchor="end">{val}</text>
          ))}
          {rects.map((r) => (
            <rect key={r.key} x={r.x} y={r.y} width={r.w} height={r.h} fill={r.fill} fillOpacity={r.opacity} rx={2} />
          ))}
          {scLabels.map((l) => (
            <text key={l.key} x={l.x} y={l.y} fontSize={10} fill={l.fill} textAnchor="middle" fontWeight="bold">{l.text}</text>
          ))}
          {dtLabels.map((l) => (
            <text key={l.key} x={l.x} y={l.y} fontSize={9} fill={l.fill} textAnchor="middle">{l.text}</text>
          ))}
        </svg>
      </div>
    </div>
