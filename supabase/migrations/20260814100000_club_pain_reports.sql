-- ═════════════════════════════════════════════════════════════════════════════
-- get_club_pain_reports : fil des déclarations de douleur individuelles,
-- groupées par soumission, pour l'onglet Infirmerie du Pôle Performance.
--
-- Distinct de get_pain_signals (20260813140000) : ce dernier agrège sur un
-- seuil et une fenêtre glissante pour repérer une récurrence — « 4
-- signalements ischio gauche en 18 jours ». Ici, le staff veut voir CE QUI a
-- été déclaré, dans l'ordre, y compris un signalement isolé qui n'atteint
-- jamais le seuil. Les deux vues coexistent, elles ne se remplacent pas.
--
-- Même repli club que get_pain_signals : pain_reports ne porte pas de club,
-- on remonte par players.club_id -> teams.club_id.
-- ═════════════════════════════════════════════════════════════════════════════
...
AS $fn$
BEGIN
  -- Déclarations de douleur = donnée de santé : même garde que get_pain_signals
  -- et get_training_load, `viewer` reste dehors.
  ...
  FROM (
    SELECT
      ...
      MAX(pr.training_id)       AS training_id,
      jsonb_agg(
...
BEGIN
  ...
  IF (SELECT pg_get_functiondef(p.oid) FROM pg_proc p
    ...
    RAISE EXCEPTION 'get_club_pain_reports n''utilise pas la garde de donnees de sante';
  END IF;
